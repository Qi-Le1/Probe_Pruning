...
def cal_intersection_ratio(output, probe, weight, pruning_module, multiple):
    ...
    cur_bsz_fullinf_metric = cal_prune_metric(output, weight, cfg['prune_metric'])
    cur_bsz_probe_out_dim_indices, cur_bsz_prune_out_dim_indices = pruning_module.sort_probe_mlp_metric(cur_bsz_fullinf_metric, multiple)
    intersection_ratios = []
    
    for i in range(bsz):
        # Convert tensors to sets for intersection operation
        set_optimal = set(optimal_prune_out_dim_indices[i].cpu().numpy())
        set_cur_bsz = set(cur_bsz_prune_out_dim_indices.cpu().numpy().flatten())
        intersection = set_optimal.intersection(set_cur_bsz)
        
        intersection_ratio = len(intersection) / len(set_optimal)
        intersection_ratios.append(intersection_ratio)
    cur_bsz_mean_intersection_ratio = sum(intersection_ratios) / len(intersection_ratios)
    ...
    for i in range(bsz):
        # Convert tensors to sets for intersection operation
        set_optimal = set(optimal_prune_out_dim_indices[i].cpu().numpy())
        set_cur_bsz = set(prune_out_dim_indices.cpu().numpy().flatten())
        intersection = set_optimal.intersection(set_cur_bsz)
        # print('intersection', len(intersection), len(set_optimal), len(set_cur_bsz))
        intersection_ratio = len(intersection) / len(set_optimal)
        intersection_ratios.append(intersection_ratio)
    # print('intersection_ratios', intersection_ratios, sum(intersection_ratios),  len(intersection_ratios))
    probe_mean_intersection_ratio = sum(intersection_ratios) / len(intersection_ratios)
    ...
    return cur_bsz_mean_intersection_ratio, probe_mean_intersection_ratio
