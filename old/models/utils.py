...
class ChannelDeletor():
    ...
    def __init__(self):
        ...
        self.pq_indices = []
        self.PQ_trans_delete_ratio_list = []
        return  
    ...
    def delete_channel_by_PQ_index(self, tensor):
        ...
        # shrink the whole column
            ...
            if cfg['batch_deletion'] == 'PQ':
                ...
                # Compute the union across all rows
                union_tensor = indices_to_prune[0]
                for row in indices_to_prune[1:]:
                    union_tensor = torch.tensor(list(set(union_tensor.tolist()) | set(row.tolist())))
                    
                    # Update the frequency counter
                    index_frequency.update(row.tolist())

                print('union_tensor', len(union_tensor), tensor.shape[1])

                # If the condition is met, identify the index with the least frequency and exclude it
                if len(union_tensor) == tensor.shape[1]:
                    # Find the channel with the least frequency
                    least_common_channel = index_frequency.most_common()[-1][0]
                    # print('zz', index_frequency, least_common_channel)
                    # Exclude the least common channel from union_tensor
                    union_tensor = torch.tensor([idx for idx in union_tensor if idx != least_common_channel])
                self.delete_channel_ratio = len(union_tensor) / tensor.shape[1]

                select_channels = list(set(range(tensor.shape[1])) - set(union_tensor.tolist()))
                select_channels.sort()
        ...
        select_channels = list(set(range(tensor.shape[1])))
        if tensor.dim() > 2:
